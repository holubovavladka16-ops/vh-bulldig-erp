...
export function PhotoDocumentView({
  ...
  async function handleShareMessenger() {
    try {
      const result = await shareToMessenger(shareText, mapUrl)
      if (result === 'copied') {
        window.alert('Zpráva byla zkopírována do schránky – vložte ji prosím do Messengeru.')
      }
      if (result !== 'cancelled') await logShare('messenger')
    } catch {
        ...
        <>
          ...
          <div className="border-t border-[var(--accent-primary)]/20 px-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <button
                type="button"
                onClick={() => void handleShareMessenger()}
