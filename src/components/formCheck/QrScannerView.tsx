...
export function QrScannerView({
  ...
  const camera = useCameraStream({
    enabled: active && !resolving,
    facingMode: 'environment',
    moduleId: 'kontrola-formulare-qr',
  })
  const scanner = useQrScanner({
    enabled: active && !resolving && camera.isStreamReady,
    videoRef: camera.videoRef,
    onScan,
  })

  const displayError = error?.message ?? camera.errorMessage ?? scanner.error ?? null
  ...
  return (
    <Card className="overflow-hidden" padding={false}>
      <div className="relative aspect-[3/4] w-full max-w-lg mx-auto bg-black/40 sm:aspect-[4/3]">
        ...
        {scanner.scanning && camera.isStreamReady && !resolving && (
