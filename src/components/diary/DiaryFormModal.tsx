...
import { Images, Loader2, MapPin, Trash2, X } from 'lucide-react'
...
import { DiaryGpsPhotoPicker } from '@/components/diary/DiaryGpsPhotoPicker'
...
export function DiaryFormModal({
  ...
  const [linkedPhotos, setLinkedPhotos] = useState<GpsPhoto[]>([])
  ...
  function buildPayload(): ConstructionDiaryCreateInput | null {
    ...
    return {
      ...
      linked_photo_ids: linkedPhotos.map((photo) => photo.id),
    }
  }
  ...
  function handlePhotosPicked(photos: GpsPhoto[]) {
    setLinkedPhotos((prev) => {
      const existingIds = new Set(prev.map((p) => p.id))
      const additions = photos.filter((p) => !existingIds.has(p.id))
      return [...prev, ...additions]
    })
    setPhotoPickerOpen(false)
  }

  function handleRemoveLinkedPhoto(photoId: string) {
  ...
  return (
    <div className="modal-overlay">
              ...
              <Card className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Button type="button" variant="secondary" size="sm" onClick={() => setPhotoPickerOpen(true)}>
                    ...
                    Přidat foto z GPS fotodokumentace
                  </Button>
                </div>
                <p className="text-xs text-theme-muted">
                  Fotografie se nenahrávají z telefonu ani z galerie zařízení – vybírají se výhradně z existující
                  galerie modulu Fotodokumentace s GPS a zachovávají si své původní GPS souřadnice, adresu, datum,
                  čas i autora.
                </p>

                {linkedPhotos.length === 0 ? (
                  <p className="text-sm text-theme-muted">Zatím nejsou přiřazené žádné fotografie.</p>
                ) : (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {linkedPhotos.map((photo) => (
                      <div key={photo.id} className="rounded-xl border border-[var(--border-glass)] p-2">
                        <a href={getGpsPhotoUrl(photo.file_path)} target="_blank" rel="noreferrer">
                          <img
                        ...
                        {photo.creator_name && (
                          <p className="text-xs text-theme-muted">Autor: {photo.creator_name}</p>
                        )}
                        <Button
                          type="button"
                          variant="danger"
                          size="sm"
                          className="mt-2"
                          onClick={() => handleRemoveLinkedPhoto(photo.id)}
                        >
      ...
      {photoPickerOpen && orderId && (
        <DiaryGpsPhotoPicker
          orderId={orderId}
          currentEntryId={initial?.id}
          onConfirm={handlePhotosPicked}
