...
import { X, Eye, Printer, FileDown, Mail, MessageCircle, Send, FileText } from 'lucide-react'
...
import { generatePayrollSlipPdfBlob, getPayrollSlipPdfFilename } from '@/lib/payroll/payrollPdf'
import {
...
export function PayrollSlipDetailModal({ workerId, period, onClose }: PayrollSlipDetailModalProps) {
  ...
  const generatePdf = useCallback(async () => {
    if (!detail) throw new Error('Výplatní páska není k dispozici.')
    return generatePayrollSlipPdfBlob(detail, company)
  }, [detail, company])

  const pdfFileName = useMemo(
    () => (detail ? getPayrollSlipPdfFilename(detail) : 'vyplatni_paska.pdf'),
    [detail]
  )

  const pdfTitle = useMemo(
    () => (detail ? buildPayrollSlipTitle(detail) : 'Výplatní páska'),
    [detail]
  )
  ...
  function share(channel: 'whatsapp' | 'messenger' | 'email') {
    ...
    if (channel === 'whatsapp') {
      ...
      shareToMessenger(text)
        ...
        .catch(() => window.alert('Sdílení do Messengeru se nezdařilo.'))
    } else {
                ...
                {mobilePdf ? (
                  <div className="mb-4">
                    ...
                    <p className="mt-2 text-xs text-theme-muted">
                      Otevře náhled PDF se stažením, sdílením a tiskem – bez systémového dialogu tisku.
                    </p>
                  </div>
                ) : (
