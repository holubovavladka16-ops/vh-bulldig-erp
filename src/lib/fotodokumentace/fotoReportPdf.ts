...
import { buildFotoReportHtml } from '@/lib/fotodokumentace/fotoReport'
import {
  assertValidPdfBlob,
  downloadPdfBlob,
  pdfBlobToFile,
...
export const FOTO_REPORT_ERP7_EXTRA_STYLES = `
  ...
  .doc-gps-a4-body .doc-table-gps tr:nth-child(even) td {
    background: #f8fafc;
  }
  ...
  .doc-gps-a4-body .doc-photo-map-empty {
    display: flex;
    align-items: center;
    justify-content: center;
    min-height: 24mm;
  ...
  @media print {
    html, body.single-page, .pdf-page {
      overflow: hidden !important;
      height: 297mm !important;
      max-height: 297mm !important;
    }
...
function buildFotoReportWatermarkHtml(company: CompanyHeader): string {
...
function buildFotoReportHeaderHtml(
...
function buildFotoReportFooterHtml(company: CompanyHeader, createdAt: string): string {
...
function buildSingleFotoPageHtml(
  foto: FotoDokument,
  company: CompanyHeader,
  createdAt: string
): string {
  const title = 'GPS fotodoklad – stavební dokumentace'
  const documentNumber = `FOTO-${foto.id.slice(0, 8).toUpperCase()}`
  const meta = { title, documentNumber, createdAt }
  const inner = `${buildFotoReportHeaderHtml(company, meta)}${buildFotoReportHtml(foto)}${buildFotoReportFooterHtml(company, createdAt)}`
  const watermark = buildFotoReportWatermarkHtml(company)
...
export function buildFotoReportPrintDocument(
  foto: FotoDokument,
  ...
  const title = 'GPS fotodoklad – stavební dokumentace'
  const styles = getProfessionalDocumentStyles(FOTO_REPORT_ERP7_EXTRA_STYLES)
  const page = buildSingleFotoPageHtml(foto, co, createdAt)
...
<body class="has-doc-footer single-page">
  ${page}
</body>
</html>`
}

export function buildFotoReportBulkPrintDocument(
  ...
  const title = 'GPS fotodoklad – stavební dokumentace'
  const styles = getProfessionalDocumentStyles(FOTO_REPORT_ERP7_EXTRA_STYLES)
  const pages = fotografie.map((foto) => buildSingleFotoPageHtml(foto, co, createdAt)).join('\n')
...
<body class="has-doc-footer multi-page">
  ${pages}
</body>
</html>`
}
...
function enforcePdfImageDimensions(doc: Document): void {
  ...
  doc.querySelectorAll<HTMLElement>('.pdf-page').forEach((page) => {
...
async function capturePageToPdfBlob(pageEl: HTMLElement, html2pdf: Html2PdfFactory): Promise<Blob> {
...
/** ERP 7 html2pdf capture – pevná A4 stránka (.pdf-page). */
export async function htmlToFotoReportPdfBlob(html: string): Promise<Blob> {
  ...
  try {
    ...
    if (pageEls.length === 0) {
      throw new Error('PDF dokument neobsahuje stránku.')
    }

    const html2pdf = await getHtml2PdfFactory()

    if (pageEls.length === 1) {
      const pdfBlob = await capturePageToPdfBlob(pageEls[0], html2pdf)
    ...
    for (const pageEl of pageEls) {
      pageBlobs.push(await capturePageToPdfBlob(pageEl, html2pdf))
    }

    const combined = new (await import('jspdf')).jsPDF({
      unit: 'mm',
      format: 'a4',
    ...
    for (let i = 0; i < pageBlobs.length; i++) {
      ...
      combined.addImage(dataUrl, 'JPEG', 0, 0, A4_WIDTH_MM, A4_HEIGHT_MM, undefined, 'FAST')
    }

    const pdfBlob = combined.output('blob') as Blob
    const result = pdfBlob.type === 'application/pdf' ? pdfBlob : new Blob([pdfBlob], { type: 'application/pdf' })
    await assertValidPdfBlob(result)
    return result
  } finally {
    iframe.remove()
  }
...
async function blobToDataUrl(blob: Blob): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error('Nelze načíst PDF stránku.'))
    reader.readAsDataURL(blob)
  })
}

export function getFotoReportPdfFileName(foto: FotoDokument): string {
  return sanitizePdfFileName(`gps-fotodoklad_${foto.captured_date}_${foto.id.slice(0, 8)}.pdf`)
}

export async function buildFotoReportPdfBlob(
  foto: FotoDokument,
  company?: CompanyHeader | CompanySettings | null
): Promise<Blob> {
  const html = buildFotoReportPrintDocument(foto, company)
  return htmlToFotoReportPdfBlob(html)
}

export async function buildFotoReportBulkPdfBlob(
  fotografie: FotoDokument[],
  company?: CompanyHeader | CompanySettings | null
): Promise<Blob> {
  ...
  if (fotografie.length === 1) {
    return buildFotoReportPdfBlob(fotografie[0], company)
  }
  const html = buildFotoReportBulkPrintDocument(fotografie, company)
  return htmlToFotoReportPdfBlob(html)
}

export async function downloadFotoReportPdf(
  foto: FotoDokument,
  company?: CompanyHeader | CompanySettings | null
): Promise<void> {
  const pdfBlob = await buildFotoReportPdfBlob(foto, company)
  downloadPdfBlob(pdfBlob, getFotoReportPdfFileName(foto))
}

export async function openFotoReportPdfPreview(
  foto: FotoDokument,
  company?: CompanyHeader | CompanySettings | null
): Promise<void> {
  const html = buildFotoReportPrintDocument(foto, company)
  const pdfBlob = await buildFotoReportPdfBlob(foto, company)
  const fileName = getFotoReportPdfFileName(foto)
...
export function printFotoReport(foto: FotoDokument, company?: CompanyHeader | CompanySettings | null): void {
  void withPdfGeneratingOverlay(async () => {
    await openFotoReportPdfPreview(foto, company)
  }).catch((err) => {
...
export async function createFotoReportPdfFile(
  foto: FotoDokument,
  company?: CompanyHeader | CompanySettings | null
): Promise<File> {
  const pdfBlob = await buildFotoReportPdfBlob(foto, company)
  return pdfBlobToFile(pdfBlob, getFotoReportPdfFileName(foto))
}
