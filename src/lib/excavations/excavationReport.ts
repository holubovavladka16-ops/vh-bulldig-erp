...
export function buildExcavationReportHtml(route: ExcavationRoute, company?: CompanyHeader | null): string {
  const mapUrl = getRouteMapUrl(route.points)
  const center = route.points[Math.floor(route.points.length / 2)] ?? route.points[0]
  const mapImage = center
    ? getStaticMapImageUrl(center.lat, center.lng, 720, 240)
    : ''

  const pointsList = route.points
    .map((p, i) => {
      const label = p.label ? ` – ${escHtml(p.label)}` : ''
      const accuracy = p.accuracy != null ? `, přesnost ±${p.accuracy < 10 ? p.accuracy.toFixed(1) : Math.round(p.accuracy)} m` : ''
  ...
  const segmentsList = getRouteSegments(route.points)
    .map(
      (seg) =>
        `<li>Bod ${seg.fromIndex + 1} → Bod ${seg.toIndex + 1}: ${formatSegmentLength(seg.meters)} m</li>`
    )
    .join('')

  return `
    <div class="report">
      ${buildCompanyHeaderHtml(company, 'Měření trasy výkopu')}
      ...
      <p class="footer">Vygenerováno z ERP VH Bulldig · ${escHtml(formatDateTime(new Date().toISOString()))}</p>
    </div>
  `
}
...
export function downloadExcavationReport(route: ExcavationRoute, company?: CompanyHeader | null): void {
  downloadHtmlDocument(
    buildPrintDocument(`Výkop ${route.name}`, buildExcavationReportHtml(route, company)),
