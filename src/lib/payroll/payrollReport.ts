...
export function buildPayrollSlipDocument(
  ...
  const content = `${buildProfessionalDocumentHeader(headerCompany, meta)}${buildPayrollSlipBodyHtml(detail, company)}${buildProfessionalDocumentFooter(headerCompany, createdAt, { pdfExportMode: options?.pdfExportMode })}`
