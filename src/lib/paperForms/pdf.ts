...
import { formatSnapshotDate, formatSnapshotEmploymentType } from '@/lib/paperForms/workerSnapshot'
...
export async function buildPaperMonthlyFormPdfBlob(
  ...
  const infoRows: Array<[string, string]> = [
    ['Jméno', ws?.first_name ?? ''],
    ['Příjmení', ws?.last_name ?? ''],
    ['Datum narození', formatSnapshotDate(ws?.birth_date)],
    ['Adresa', ws?.address ?? ''],
    ['Datum nástupu', formatSnapshotDate(ws?.start_date)],
    ['Pozice', ws?.position ?? ''],
    ['Pracovní poměr', formatSnapshotEmploymentType(ws?.employment_type)],
