...
export async function createPaperMonthlyFormForWorker(
  ...
  if (error) {
    ...
    if (match) {
      const dup = new Error('DUPLICATE_ACTIVE_FORM')
      ;(dup as Error & { existingFormId: string }).existingFormId = match[1]!
      throw dup
    }
...
export async function createPaperMonthlyFormReplacement(
  originalFormId: string,
  supervisorId?: string | null
): Promise<string> {
  const { data, error } = await supabase.rpc('create_paper_monthly_form_replacement', {
...
export function parseDuplicatePaperFormError(err: unknown): string | null {
