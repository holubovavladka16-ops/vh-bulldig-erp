...
export interface WebShareInput {
  title?: string
  text?: string
  url?: string
  files?: File[]
...
function canSharePayload(input: WebShareInput): boolean {
...
export async function shareWithFiles(input: WebShareInput): Promise<WebShareResult> {
  ...
  if (files.length > 0 && !canSharePayload(input)) {
    return 'unsupported'
  }
...
export async function shareToMessenger(text: string, url?: string): Promise<MessengerShareResult> {
  const result = await shareWithFiles(url ? { text, url } : { text })
